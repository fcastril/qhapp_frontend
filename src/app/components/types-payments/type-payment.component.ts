...
export class TypePaymentComponent implements OnInit {


  id: string;
  title= 'Formas de Pago';
  controller= 'typespayments';
  navigateToPage='/types-payments';
  subtitle: string;
  reg = new TypePaymentModel();
    ...
    Swal.fire(
      ...
      if (result.isConfirmed) {
        if (this.reg.idTypePayment === 0){
          ...
          this.api.put(this.controller,this.reg, this.reg.idTypePayment).subscribe(
            (resp: any)=>{
            if (resp.error) {
