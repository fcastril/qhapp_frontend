...
export class CivilStatusComponent implements OnInit {
  ...
  navigateToPage='/civil-statuses';
  subtitle: string;
  reg = new CivilStatusModel();
    ...
    Swal.fire(
      ...
      if (result.isConfirmed) {
        if (this.reg.idCivilStatus === 0){
          ...
          this.api.put(this.controller,this.reg, this.reg.idCivilStatus).subscribe(
            (resp: any)=>{
            if (resp.error) {
