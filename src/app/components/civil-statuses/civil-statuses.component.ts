...
@Component({
  ...
  styleUrls: ['./civil-statuses.component.scss']
})
export class CivilStatusesComponent implements OnInit {

  
  regs: CivilStatusModel[] = [];
  searchText: '';
  ...
  search(){
    this.api.getSearch('civilstatus', this.searchText).subscribe(
      (resp: any)=>{
        this.regs = resp;
      }
    );
  ...
  register(id: number){
    if (id===0){ // Registro Nuevo
      this.route.navigate(['civil-status','0']);
    }
  ...
  delete(id: number, idx: number){
    Swal.fire(
      ...
      if (result.isConfirmed) {
        this.api.delete('civilstatus', id).subscribe(
          (resp:any) =>
          {
            if (resp.error) {
