...
export class ListDailyAgendaComponent implements OnInit {
  ...
  search() {
    let searchTextUnion: string =
      this.searchText + "|" + this.idTypeAgenda.toString();
    this.api
      .getSearch("DailyAgenda", searchTextUnion)
      .subscribe((resp: DailyAgendaModel[]) => {
        this.regs = resp;
        this.regs.forEach((x) => {
  ...
  searchStatus() {
    this.api.getSearch("StatusAppointments", "").subscribe((resp: any) => {
    ...
    this.modalService
      ...
      .result.then(
        (result) => {
          ...
          this.api
            .getId("AppointmentRecords", idAppointmentRecord)
            .subscribe((resp: any) => {
  ...
  update(appointment: AppointmentRecordModel, idAppointmentRecord: any) {
    this.api
      .put("AppointmentRecords", this.appointment, Number(idAppointmentRecord))
      .subscribe((resp: any) => {
        if (resp.error) {
          Swal.fire(
  ...
  changePrice(idAppointmentRecord, contentPrice) {
    this.modalService
      .open(contentPrice, { ariaLabelledBy: "modal-basic-title" })
      .result.then(
        (result) => {
