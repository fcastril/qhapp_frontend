...
import { AddNoteModel } from '../../models/add-note.model';
...
export class AddNoteComponent implements OnInit {
  ...
  controller= 'AddNotes';
  navigateToPage='/addnotes';
  subtitle: string;
  reg = new AddNoteModel();
    ...
    Swal.fire(
      ...
      if (result.isConfirmed) {
        if (this.reg.idAddNote === 0){
          ...
          this.api.put(this.controller,this.reg, this.reg.idAddNote).subscribe(
            (resp: any)=>{
            if (resp.error) {
