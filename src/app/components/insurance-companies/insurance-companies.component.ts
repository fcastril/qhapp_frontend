...
export class InsuranceCompaniesComponent implements OnInit {
  ...
  search(){
    this.api.getSearch('insurancecompanies', this.searchText).subscribe(
      (resp: any)=>{
        this.regs = resp;
      }
    );
  ...
  register(id: number){
    if (id===0){ // Registro Nuevo
      this.route.navigate(['insurance-company','0']);
    }
  ...
  delete(id: number, idx: number){
    Swal.fire(
      ...
      if (result.isConfirmed) {
        this.api.delete('insurancecompanies', id).subscribe(
          (resp:any) =>
          {
            if (resp.error) {
