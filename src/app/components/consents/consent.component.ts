...
export class ConsentComponent implements OnInit {
  ...
  controller= 'Consents';
  navigateToPage='/consents';
  subtitle: string;
  reg = new ConsentModel();
    ...
    Swal.fire(
      ...
      if (result.isConfirmed) {
        if (this.reg.idConsent === 0){
          ...
          this.api.put(this.controller,this.reg, this.reg.idConsent).subscribe(
            (resp: any)=>{
            if (resp.error) {
