...
import { ViewListPricesModel } from '../../models/v_ListPrices.model';
...
export class ListPricesComponent implements OnInit {
  ...
  search(){
    this.api.getSearch('listPrices', this.searchText).subscribe(
      (resp: any)=>{
        this.regs = resp;
      }
    );
  ...
  register(id: number){
    if (id===0){ // Registro Nuevo
      this.route.navigate(['list-price','0']);
    }
  ...
  delete(id: number, idx: number){
    Swal.fire(
      ...
      if (result.isConfirmed) {
        this.api.delete('listPrices', id).subscribe(
          (resp:any) =>
          {
            if (resp.error) {
