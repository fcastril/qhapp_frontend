...
export class MeasureControlComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.api.get("AddNotes").subscribe((resp: any) => {
      this.listNotes = resp;
    ...
    this.api.getSearch(this.controller, this.id).subscribe((resp: any) => {
      this.regs = resp;
      console.log(resp);
  ...
  saveRegister(idDetail, idx) {
    ...
    this.api
      .put(this.controller, this.regEdit, this.regEdit.idDetailMeasureControl)
      .subscribe((resp: any) => {
        if (resp.error) {
          Swal.fire(
          ...
          Swal.fire(
            "Registro realizado",
            "Se realizo la actualización correctamente",
            "success"
          );
          this.ngOnInit();
        }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (this.reg1.idDetailMeasureControl === 0) {
          this.api.post(this.controller, this.reg1).subscribe((resp: any) => {
            if (resp.error) {
              Swal.fire(
          ...
          this.api
            .put(this.controller, this.reg1, this.reg1.idDetailMeasureControl)
            .subscribe((resp: any) => {
              if (resp.error) {
                Swal.fire(
                  "Error al actualizar el Registro",
