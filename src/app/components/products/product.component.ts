...
import { ProductBasicModel } from "../../models/product-basic.model";
import { SIIGOFEProductRequestModel } from "../../models/siigofeProductRequest.model";
  ...
  ngOnInit(): void {
    ...
    if (this.id === "0") {
      ...
      this.api
        ...
        .subscribe((resp: ProductModel) => (this.reg = resp));
    }

    // se carga la lista del select para Tipo Producto
    this.api.get("TypesProducts").subscribe((resp: any) => {
    ...
    this.api.get("Uens").subscribe((resp: any) => {
      this.listUEMs = resp;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (this.reg.idProduct === 0) {
          ...
          this.api
            .put(this.controller, this.reg, this.reg.idProduct)
            .subscribe((resp: any) => {
              if (!resp) {
                Swal.fire(
        ...
        let productBasic: ProductBasicModel = new ProductBasicModel();
        productBasic.code = this.reg.codeProduct;
        productBasic.name = this.reg.descriptionProduct;

        let siigoFEProductRequestModel: SIIGOFEProductRequestModel =
          new SIIGOFEProductRequestModel();
        siigoFEProductRequestModel.username = environment.Username;
        siigoFEProductRequestModel.access_key = environment.AccessKey;
        siigoFEProductRequestModel.uri = environment.urlSIIGOFE;
        siigoFEProductRequestModel.productBasic = productBasic;

        this.api
          .post("ElectronicInvoice/PostProduct", siigoFEProductRequestModel)
          .subscribe((resp: ResponseModel) => {
            if (!resp.response) {
              Swal.fire(
                "Error al actualizar el Registro en SIIGO",
                "Se presentó un error al actualizar el registro en SIIGO",
                "error"
              );
            } else {
              console.log("Registro actualizado en SIIGO Correctamente");
            }
          });
      }
    });
